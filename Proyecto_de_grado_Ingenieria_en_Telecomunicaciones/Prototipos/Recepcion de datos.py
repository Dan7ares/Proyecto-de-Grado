...
import warnings
import os # Importar el módulo os para operaciones con directorios
import re # Importar el módulo re para expresiones regulares
import glob # Importar el módulo glob para encontrar archivos por patrón
...
def _leer_un_solo_archivo_excel(file_path):
    ...
    # 1. Intentar leer la hoja 'datos'
    try:
        df_single_file = pd.read_excel(file_path)
        sheet_found = True
        print(f"  - Archivo procesado {os.path.basename(file_path)}.")
    except ValueError:
        print(f"  - datos no encontrada en {os.path.basename(file_path)}.")
        # 2. Si 'datos' no existe, iterar por todas las hojas
        try:
            xls = pd.ExcelFile(file_path)
            for sheet_name_iter in xls.sheet_names:
                try:
                    df_temp = pd.read_excel(file_path, sheet_name=sheet_name_iter)
                    ...
                    print(f"  - Error al leer la hoja '{sheet_name_iter}' de {os.path.basename(file_path)}: {e}")
 ...
 # Ordenar las columnas potenciales por el número extraído o por orden alfabético si no hay número
    ...
    if len(potential_freq_cols) == len(potential_amp_cols) and len(potential_freq_cols) > 0:
        for freq_col, amp_col in zip(potential_freq_cols, potential_amp_cols):
            frecuencias = df_single_file[freq_col].apply(helpers.transform_dato).dropna().values
            amplitudes = df_single_file[amp_col].apply(helpers.transform_dato).dropna().values 
