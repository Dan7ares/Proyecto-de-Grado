...
def evaluar_modelo(modelo, X_prueba, y_prueba, guardar_roc=True, ruta_salida_roc=None):
    """
    Evalua el rendimiento del modelo de clasificacion, imprime exactitud, genera informe,
    calcula y grafica la curva ROC con su AUC. Retorna la matriz de confusion y el AUC.
    Parametros:
        modelo: modelo entrenado que soporte predict_proba
        X_prueba: DataFrame o array de features de prueba (escalados si aplica)
        y_prueba: series/array de etiquetas verdaderas
        guardar_roc: bool, si guardar la figura ROC en disco
        ruta_salida_roc: path donde guardar la figura ROC (si None se guarda en el directorio actual)
    Retorna:
        matriz_confusion, auc_score
    """
    ...
    # Calcular scores de probabilidad para ROC/AUC si el modelo lo soporta
    auc_score = None
    try:
        if hasattr(modelo, "predict_proba"):
            y_scores = modelo.predict_proba(X_prueba)[:, 1]
        elif hasattr(modelo, "decision_function"):
            y_scores = modelo.decision_function(X_prueba)
        else:
            y_scores = None

        if y_scores is not None:
            fpr, tpr, thresholds = roc_curve(y_prueba, y_scores)
            ...
            plt.plot([0, 1], [0, 1], linestyle='--', lw=1, color='gray', label='Aleatorio (AUC = 0.5)')
            plt.xlim([-0.01, 1.01])
            plt.ylim([-0.01, 1.01])
            ...
            # Guardar o mostrar
            if ruta_salida_roc is None:
                ruta_salida_roc = os.path.join(os.getcwd(), 'roc_curve.png')
            if guardar_roc:
                plt.savefig(ruta_salida_roc, dpi=200, bbox_inches='tight')
                print(f"Gráfica ROC guardada en: {ruta_salida_roc}")
            plt.show()
        else:
            print("El modelo no suministra probabilidades ni función de decisión; no se puede calcular ROC/AUC.")
    except Exception as e:
        print(f"Advertencia: No se pudo calcular la curva ROC/AUC. Error: {e}")

    return matriz_confusion, auc_score
