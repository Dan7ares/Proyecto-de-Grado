...
def curva_Pd_vs_SNR(modelo, escalador, datos_prueba, y_prueba, num_bins=30):
    """
    Calcula y grafica la curva de probabilidad de detección (Pd) vs SNR.
    Pd = TP / (TP + FN) en cada bin de SNR.
    """
    print("Generando Curva Pd vs SNR (corregida)...")
    ...
    # Definir bins uniformes en el rango de SNR
    bins = np.linspace(df_resultados['SNR'].min(), df_resultados['SNR'].max(), num_bins+1)
    df_resultados['SNR_bin'] = pd.cut(df_resultados['SNR'], bins=bins, include_lowest=True)
    ...
    for bin_interval, grupo in df_resultados.groupby('SNR_bin'):
        ...
        Pd = TP / (TP + FN) if (TP + FN) > 0 else np.nan
        snr_promedio = grupo['SNR'].mean()
        curva.append((snr_promedio, Pd))

    curva = pd.DataFrame(curva, columns=['SNR_promedio', 'Pd']).dropna().sort_values('SNR_promedio')
    ...
    if datos_ml.empty:
        ...
        if datos_ml.empty:
            ...
            if modelo_entrenado:
                ...
                curva_ROC(modelo_entrenado, escalador, X_prueba, y_prueba)
...
def curva_ROC(modelo, escalador, X_prueba, y_prueba):
    """
    Genera y grafica la curva ROC para el modelo entrenado.
    """
    if modelo is None or X_prueba.empty or y_prueba.empty:
        print("Error: Datos insuficientes para generar la curva ROC.")
        return

    if escalador is not None:
        X_prueba_escalado = pd.DataFrame(
            escalador.transform(X_prueba),
            columns=X_prueba.columns,
            index=X_prueba.index
        )
    else:
        X_prueba_escalado = X_prueba

    y_scores = modelo.predict_proba(X_prueba_escalado)[:, 1]
