...
from sklearn.svm import SVC # <-- El unico modelo importado
import threading
...
def entrenar_modelo_clasificacion(X_entrenamiento, y_entrenamiento):
    """
    Entrena un modelo de clasificacion binaria de tipo SVC.
    """
    ...
    print(f"Entrenando modelo: SVC...")
    
    # Se entrena unicamente el modelo SVC
...
def evaluar_modelo(modelo, X_prueba, y_prueba, nombre_modelo):
    """
    ...
    print(f"\nExactitud general del modelo {nombre_modelo}: {exactitud:.4f}")
...
def predecir_uso_canal(modelo, escalador, datos_nuevos):
    """
    Predice si un canal esta en uso o libre para nuevas observaciones.
    """
    if modelo is None: 
    ...
    # Calcular la caracteristica SNR si no existe en los datos
...
def ejecutar_pipeline_clasificacion():
    """
    Esta funcion contiene el flujo principal de ejecucion del pipeline de ML.
    Retorna el modelo, el escalador y la matriz de confusion.
    """
    ...
    if datos_ml.empty:
        ...
        if datos_ml.empty:
            ...
            modelo_entrenado = entrenar_modelo_clasificacion(X_entrenamiento_escalado, y_entrenamiento)
            matriz_confusion = None
            
            if modelo_entrenado:
                print(f"\n--- EVALUACION DEL MODELO SVC EN EL CONJUNTO DE PRUEBA ---")
                matriz_confusion = evaluar_modelo(modelo_entrenado, X_prueba_escalado, y_prueba, 'SVC')
...
class MLApp(tk.Tk):
    ...
    def entrenar_modelo(self):
        self.modelo_entrenado, self.escalador, self.matriz_confusion = ejecutar_pipeline_clasificacion()
        
        if self.modelo_entrenado and self.escalador:
            self.after(0, self.mostrar_matriz_confusion)
    ...
    def habilitar_app(self):
        ...
        messagebox.showinfo("Listo", "El modelo de clasificacion ha sido entrenado y esta listo para usarse. Revisa la consola para ver los resultados.")
    ...
    def mostrar_matriz_confusion(self):
        """
        Genera y muestra la matriz de confusion para el modelo.
        """
        if self.matriz_confusion is not None:
            print(f"Generando la matriz de confusion para el modelo SVC...")
            plt.figure(figsize=(6, 5))
            sns.heatmap(self.matriz_confusion, annot=True, fmt='d', cmap='Blues',
                        xticklabels=['Canal Libre (0)', 'Canal en Uso (1)'],
                        yticklabels=['Canal Libre (0)', 'Canal en Uso (1)'])
            plt.xlabel('Prediccion del Modelo')
            plt.ylabel('Valor Real')
            plt.title(f'Matriz de Confusion: SVC')
            plt.show()
